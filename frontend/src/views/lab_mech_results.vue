    ...
    <main class="mx-auto w-[94vw] max-w-[1500px] pt-4 pb-6">
      <section class="bg-white border border-slate-200 overflow-hidden">
        <div class="px-4 py-3 flex items-center justify-between border-b border-slate-200 gap-3 flex-wrap">
          <div>
            <div class="font-semibold text-slate-900">機械性質試驗紀錄表判定結果</div>
            <div class="text-xs text-slate-500 mt-1">此頁僅供瀏覽，顯示已完成判定的機械性質試驗紀錄表。</div>
          </div>
...
async function reload() {
  loading.value = true
  try {
    const resp = await apiFetch('/ai/api/lab/mech/reports/judgment/results')
